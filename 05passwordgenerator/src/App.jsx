...
export default function App() {
  ...
  const generatePassword = useCallback(() => {
    ...
    // updating str if the below conditions
    if (numberAllowed) str += "0123456789";
    if (charAllowed) str += "!@#$%^&*()_+";

    // generating random characters and appending it to the actual password string
    ...
    <div className="w-full max-w-md mx-auto shadow-md rounded-lg px-4 py-3 my-8 bg-gray-800">
      ...
      <div className="flex text-sm gap-x-2 text-white">
        <div className="flex items-center gap-x-1">
          <input type="range" name="" id="" min={6} max={20} className="cursor-pointer" onChange={(e) => setLength(e.target.value)} />
        ...
        <div className="flex items-center gap-x-1">
          <input type="checkbox" name="" defaultChecked={charAllowed} onChange={() => setCharAllowed((prev) => !prev)} id="" />
